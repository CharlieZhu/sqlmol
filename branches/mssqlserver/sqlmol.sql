...
BEGIN
	...
	*/
	...
	declare @cyclic table (
		cycid   smallint,
		nodeid1 smallint not null,
		nodeid2 smallint null,
		bond    char(2) not null default '-',
		primary key (cycid, nodeid1)
	)
	...
	BEGIN
		...
		BEGIN
			-- print @code
			declare @cycid as varchar(100)
            select @cycid = CAST( @code as smallint)
			
			if exists( select cycid from @cyclic where cycid=@cycid and nodeid2 is null )
				update @cyclic 
				set 
				    nodeid2= @api,
				    bond= case 
				...
				where cycid=@cycid  and nodeid2 is null
			else
